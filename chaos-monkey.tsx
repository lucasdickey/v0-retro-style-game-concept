...
export default function ChaosMonkey() {
  ...
  useEffect(() => {
    ...
    let tokens: { x: number; y: number }[] = []
    let powerUps: { x: number; y: number; active: boolean }[] = []
    let apes: { x: number; y: number; direction: { x: number; y: number } }[] = []
    let powerMode = false
    ...
    function moveApes() {
      try {
        ...
        for (let i = 0; i < apes.length; i++) {
          ...
          const leftBlocked = ape.x <= 0 || !maze[ape.y] || maze[ape.y][ape.x - 1] === 1
          const rightBlocked = ape.x >= GRID_WIDTH - 1 || !maze[ape.y] || maze[ape.y][ape.x + 1] === 1
          const upBlocked = ape.y <= 0 || !maze[ape.y - 1] || maze[ape.y - 1][ape.x] === 1
          const downBlocked = ape.y >= GRID_HEIGHT - 1 || !maze[ape.y + 1] || maze[ape.y + 1][ape.x] === 1

          // Count how many directions are blocked
          const blockedCount = [leftBlocked, rightBlocked, upBlocked, downBlocked].filter((blocked) => blocked).length
          const isStuck = blockedCount >= 2

          // In power mode, apes try to run away from player
          // Otherwise, they aggressively pursue the player
          const targetX = powerMode ? (ape.x < player.x ? ape.x - 1 : ape.x + 1) : player.x
          const targetY = powerMode ? (ape.y < player.y ? ape.y - 1 : ape.y + 1) : player.y
          ...
          if (possibleMoves.length > 0) {
            ...
            if (!useRandomMove) {
              ...
              ape.direction = possibleMoves[Math.floor(Math.random() * possibleMoves.length)]
            }
          } else if (!ape.direction || typeof ape.direction.x !== "number" || typeof ape.direction.y !== "number") {
      ...
      try {
        ...
        if (powerUps) {
          for (let i = powerUps.length - 1; i >= 0; i--) {
            ...
            if (powerUp && powerUp.x === player.x && powerUp.y === player.y && powerUp.active) {
              powerUps.splice(i, 1)
              powerMode = true
    ...
    function draw() {
      try {
        ...
        if (powerMode) {
          ctx.fillStyle = "#00FFFF"
          ctx.fillText(`POWER: ${Math.ceil(powerModeTimer / 30)}`, canvas.width / 2 - 80, 30)
        }
      } catch (error) {
  ...
  return (
    <div className="flex flex-col items-center justify-center w-full max-w-[800px] mx-auto">
      <h1 className="text-4xl font-bold mb-4 text-center font-press-start">CHAOS MONKEY</h1>

      {!gameStarted ? (
        <div className="flex flex-col items-center justify-center p-8 bg-black text-white w-full h-[800px]">
        ...
        <canvas ref={canvasRef} width={800} height={800} className="border border-gray-800 bg-black" tabIndex={0} />
      )}
